...
class Vehiculos {
  ...
  listarTodos() {
    this.todos.forEach(element => {
      if (element instanceof Auto) {
        console.log(`Marca: ${element.marca} // Modelo: ${element.modelo} // Puertas: ${element.puertas}  // Precio: ${this.precioATexto(element.precio)}`);
      } else if (element instanceof Moto) {
        console.log(`Marca: ${element.marca} // Modelo: ${element.modelo} // Cilindrada: ${element.cilindrada}  // Precio: ${this.precioATexto(element.precio)}`);
      } else {
        console.log("Error!");
      }
    })
  ...
  contieneLetra(letra) {
    ...
    if (vehiculo) {
      console.log(`Vehículo que contiene en el modelo la letra '${letra}': ${vehiculo.marca} ${vehiculo.modelo} ${this.precioATexto(vehiculo.precio)}`);
    } else {
      console.log('Error!');
    }
  }
  ...
  precioATexto(precio) {
    let price = new Intl.NumberFormat("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(precio);
...
const vehiculos = new Vehiculos(datos);
vehiculos.printAllData()

/*
### NOTA

La funcion Intl.NumberFormat no funciona correctamente
ejecutando directamente con Node (debido a su instalacion por defecto), pero si desde la consola de un navegador.
Tuve que forzar el formato del precio en 'en-US' y posteriormente
reemplazar los simbolos convenientes para la correcta visualizacion. (funcion precioATexto)
*/